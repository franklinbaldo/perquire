...
# Import the CLI entry point from lean_main
from perquire.cli.lean_main import cli as lean_cli_app
...
def test_cli_entry_point(runner):
    ...
    result = runner.invoke(lean_cli_app, ['--help'])
    assert result.exit_code == 0
    # Default group name for cli in lean_main.py is 'cli', so usage shows 'cli'
...
def test_cli_providers_command(runner):
    ...
    with patch('perquire.cli.lean_main.list_available_providers') as mock_list_providers:
        ...
        result = runner.invoke(lean_cli_app, ['providers'])
        assert result.exit_code == 0
...
def test_cli_investigate_command(runner, mock_investigator_instance, tmp_path):
    ...
    with patch('perquire.cli.main.create_investigator_from_cli_options', return_value=mock_investigator_instance) as mock_create_inv:
        result = runner.invoke(lean_cli_app, [
...
def test_cli_investigate_file_not_found(runner):
    result = runner.invoke(lean_cli_app, ['investigate', 'nonexistent.npy'])
    assert result.exit_code != 0
...
def test_cli_status_command(runner):
    ...
    with patch('perquire.cli.main.DuckDBProvider', return_value=mock_db_provider) as mock_duckdb_init:
        result = runner.invoke(lean_cli_app, ['status', '--database', 'dummy.db']) # dummy.db will be passed to DuckDBProvider
...
def test_cli_configure_command(runner, tmp_path, monkeypatch): # Added monkeypatch
    ...
    result_show = runner.invoke(lean_cli_app, ['configure', '--show'])
