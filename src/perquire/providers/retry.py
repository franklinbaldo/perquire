...
T = TypeVar("T")
_TRANSIENT_NAMES = {"RateLimitError", "ServiceUnavailableError"}
_RETRY_AFTER = re.compile(r"retry_after_seconds(?:_raw)?[\"']?\s*[:=]\s*[\"']?([0-9]+(?:\.[0-9]+)?)")


def is_transient_provider_error(error: Exception) -> bool:
    """Return true only for retryable 429/503-style provider failures."""
    if type(error).__name__ in _TRANSIENT_NAMES:
        return True
    text = str(error).lower()
    return "429" in text or "503" in text or "rate limit" in text or "temporarily unavailable" in text
...
def call_with_transient_retries(
    ...
    *,
    ...
    """Run an operation with bounded retries for transient 429/503 failures.
